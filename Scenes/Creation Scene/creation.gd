...
#puts a character limit on the length of the product name
func _on_title_text_changed(new_text: String) -> void:
	if new_text.length() > MAX_LENGTH:
		%Title.text = %Title.text.substr(0, MAX_LENGTH)
		%Title.set_caret_column(MAX_LENGTH)


#puts a character limit on the length of the slogan
func _on_slogan_text_changed(new_text: String) -> void:
	if new_text.length() > MAX_LENGTH:
		%Slogan.text = %Slogan.text.substr(0, MAX_LENGTH)
		%Slogan.set_caret_column(MAX_LENGTH)
